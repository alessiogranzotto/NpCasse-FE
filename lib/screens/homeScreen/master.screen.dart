...
class _MasterScreenState extends State<MasterScreen> {
  ...
  void handleSubMenuTap(int mainMenuIndex, int subMenuIndex) {
    setState(() {
      ...
      if (_selectedMainMenuIndex != mainMenuIndex || _selectedSubMenuIndex != subMenuIndex) {
        _selectedMainMenuIndex = mainMenuIndex;
        _selectedSubMenuIndex = subMenuIndex;
        _currentScreen = currentDestinations[mainMenuIndex].subMenus![subMenuIndex].screen;
      }
    });
  }
  ...
  int calculateSelectedIndex() {
    ...
    for (int i = 0; i < _selectedMainMenuIndex; i++) {
      if (currentDestinations[i].subMenus != null && visibleSubMenus.contains(i)) {
    ...
    if (_selectedSubMenuIndex != null) {
      selectedIndex += subMenuOffset + _selectedSubMenuIndex! + 1; // +1 for the main menu itself
    } else {
  ...
  Widget build(BuildContext context) {
    ...
    if (cUserModel == null || cSelectedUserAppInstitution == null) {
      return const Center(
          child: CircularProgressIndicator()); // Show loading spinner until data is available
    }
          ...
          Expanded(
            ...
            child: LazyIndexedStack(
              ...
              children: currentDestinations.map((menu) {
                if (menu.subMenus != null && visibleSubMenus.contains(_selectedMainMenuIndex)) {
                  // Use LazyIndexedStack for the sub-menu screens
                  return LazyIndexedStack(
                    index: _selectedSubMenuIndex ?? 0, // Default to the first submenu if none is selected
